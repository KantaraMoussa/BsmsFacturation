...
!function (NioApp, $) {
  ...
  NioApp.Select2 = function (parent = null, elm = '.js-select2') {

    let opt = {};
    if (parent)
      opt.dropdownParent = parent;
  ...
  NioApp.loadModal = (ajaxOption, opts) => {

    $.fn.modal.Constructor.prototype.enforceFocus = function () { };

    var idModal = opts.idModal || '#xmModal',
      overflowVal = $j('body').style('overflow-y'),
      idLoadContent = opts.idLoadContent || '.modal-content',
      type = opts.type;
    opts.backdrop = opts.backdrop || 'static';

    var myModal = new bootstrap.Modal(document.getElementById(idModal.substr(1)), {
      keyboard: false
    });

    myModal.show();

    if (!type || $j.trimAll(type).length == 0)
      type = 'modal-default';

    ajaxOption.afterLoad = ajaxOption.afterLoad || function () { };
    ajaxOption.onHidden = ajaxOption.onHidden || function () { };
    ajaxOption.method = ajaxOption.method || 'GET';
    ...
    $(idModal).on('shown.bs.modal', function (e) {
      ...
      $j(idModal + ' .modal-dialog').removeAttr('style')
        ...
        .addClass(type)
        .attr('id', ajaxOption.boxId || $_GET['_a'] || $_GET['try']);
    });

    $(idModal).on('hide.bs.modal', function (e) {
      $j('html,body').style('overflow-y', overflowVal);
      ajaxOption.onHidden.call(this, ajaxOption.boxId);
    });
    ...
    $j.ajax({
      ...
      onSuccess: function (data) {
        ...
        setTimeout(function () {
          $j(idModal + ' ' + idLoadContent).html(data);
          $j('html,body').style({
            'overflow': 'hidden'
          });
          ajaxOption.afterLoad.call(this, myModal, data);
        }, 1000);

      },
      beforeSend: function () {
        $j(idModal + ' ' + idLoadContent).html(
          '<h1 style="text-align:center"><img style="margin:10px;height:45px" src="'+baseUrl+'images/icons/loading.gif" /></h1>');
      },
      onError: function (error) {
        if (error.code != 0) alert(error.text);
      }
    });
